...
struct SymDst {
  enum SType { MAP_SYM, CONTROL_SYM } stype;
...
struct ControlSym : public SymDst {
  char ctrlChar;
  ControlSym() : SymDst(CONTROL_SYM) {}
  ControlSym(string cc) : SymDst(CONTROL_SYM), ctrlChar(cc[0]) {}
};
...
struct LSystem {
  ...
  LSystem() = default;
  LSystem(map<string, LProduction> prods) : prods(LEXY_MOV(prods)) {}
  LSystem(vector<LProduction> prods) : prods() {
...
struct LProdCall {
  enum PType { CALL_SYM, CONTROL_SYM } ptype;
  LProdCall(PType ptype) : ptype(ptype) {}
};

struct LControlSym : public LProdCall {
  char ctrlChar;
  LControlSym(string cc) : ctrlChar(cc[0]), LProdCall(CONTROL_SYM) {}
};
...
struct LSysCall {
  ...
  LSysCall(vector<shared_ptr<LProdCall>> calls) : calls(LEXY_MOV(calls)) {}
